...
vmfault(pagetable_t pagetable, uint64 va, int is_read)
{
  ...
  // Basic sanity check: don't handle crazy high addresses.  
  // Return 0 if it exceeds or equals MAXVA.
  if (va >= MAXVA)
    return 0;
  ...
  // Allocate a physical page in mem variable. If no mem allocated, 
  // return 0 to indicate out of memory.
  if ((mem = kalloc()) == 0) {
    return 0;
  }
